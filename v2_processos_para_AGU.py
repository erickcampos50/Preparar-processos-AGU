    ...
    for item in items:
        ...
        if extension in IMAGE_EXTS:
            with Image.open(current_path) as image:
                ...
                target_name = safe_filename(item[:22] + "-" + os.path.basename(current_path))
            ...
            with zipfile.ZipFile(current_path, 'r') as inner_zip:
                for member in inner_zip.namelist():
                    target_name = safe_filename(item[:22] + "-" + os.path.basename(member))
...
def main():
    ...
    with st.expander("Ver tutorial completo"):
        ...
        Foi preparado um tutorial em vídeo com todas as etapas. 
            ...
            Obs: a conversão dos arquivos DOC para 
            PDF não é realizada por essa ferramenta.
            Veja como converter estes arquivos no final do tutorial.
